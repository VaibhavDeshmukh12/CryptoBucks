...
const SavedBtn = ({ data }) => {
  ...
  return (
    <button className='outline-none border-0 bg-none cursor-pointer'
      onClick={(e) => handleClick(e)}>
      <svg className={`w-[1.5rem] ml-1.5 
      ${allCoins.includes(data.id) ? 'fill-cyan' : 'fill-gray-100'} hover:fill-cyan`} width="30" height="30" viewBox="0 0 30 30" fill="cyan" xmlns="http://www.w3.org/2000/svg">
          ...
          cryptoData ?
            <table className='w-full table-auto'>
              <thead className='capitalize text-base text-gray-100 
                fw-medium border-b border-gray-100'>
                <tr>
                  ...
                  cryptoData?.map((data) => {
                    return (
                      <tr key={data.id} className='text-center text-base border-b border-gray-100
                        ...
                        <td className='py-4'>{Number(data.market_cap_change_percentage_24h).toFixed(5)}%</td>
            ...
            : <div className='h-full w-full min-h-[60vh] flex justify-center items-center'>
