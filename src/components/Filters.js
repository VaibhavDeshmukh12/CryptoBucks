...
const Filters = () => {
	...
	const { setCurrency, setSortBy, resetFunc } = useContext(CryptoContext);
        ...
        flex items-center justify-between relative'>
			...
			<div className='flex mr-7'>
				...
				>
                    ...
                    flex relative items-center justify-center mr-2 font-bold'>Currency</label>
					<input ref={currencyRef} type="text" name='currency'
						placeholder='usd' className='w-16 rounded bg-gray-200
				...
				>
					...
					<select name="sortby" onClick={handleSort}
						className='rounded text-base bg-gray-200 pl-2 pr-10 
              py-0.5 leading-4 capitalize focus:outline-0' >
						<option value="market_cap_asc">market cap asc</option>
						<option value="market_cap_desc" selected>market cap desc</option>
						<option value="volume_asc">volume asc</option>
						<option value="volume_desc">volume desc</option>
						<option value="id_asc">id asc</option>
						<option value="id_desc">id desc</option>
					</select>
					<img src={selectIcon} alt="submit"
						className='w-[1.1rem] h-auto absolute 
					right-[0.1rem] top-1.5 pointer-events-none' />
				</label>
